<script lang="ts">
  ...
  let { status, collectorTotal, collectorCount, onOpenCollector, onCancel }: Props = $props()
</script>
...
<footer class="border-t bg-background/55 px-4 py-2 backdrop-blur-xl">
  <div class="grid gap-3 md:grid-cols-[1fr_auto_auto] md:items-center">
    <div class="min-w-0 space-y-2">
      <div class="flex items-center gap-2 text-sm">
        ...
        <span class="truncate text-muted-foreground"
          >{status?.message || (status?.state === 'ready' ? 'Scan complete · ready to explore' : 'Ready')}</span
        >
        {#if status?.state === 'scanning'}
    ...
    <Button variant="secondary" size="sm" type="button" onclick={onOpenCollector} class="justify-start">
      <PackageOpen class="size-4" />
      Collector: {collectorCount} / {formatBytes(collectorTotal)}
    </Button>
    {#if status?.state === 'scanning'}
      <Button variant="ghost" size="sm" type="button" onclick={onCancel} disabled={status?.state !== 'scanning'}>
