...
The workspace also carries the KFS file-search stack: `kfs-core` (types,
path policy, ranking), `kfs-crawler`, `kfs-index-sqlite` (persistent index
with a monotonic schema version, `kfs_index_sqlite::SCHEMA_VERSION`),
`kfs-daemon`, `kfs-watcher`, `kfs-provider-spotlight` (macOS `mdfind`;
other platforms return `Unsupported` rather than falling back), `kfs-napi`
(Node-API package `@kunkunsh/file-search-native`), and `kfs-cli`. It was
extracted from Kunkun's `crates/file-search` with full history and merged
into this workspace — see `docs/kfs/MIGRATION.md` for the source commit
...
deliberately not a yarn workspace member; `.prettierignore` excludes the
directory so `yarn format` can never reformat its generated bindings.

Consumed by [Kunkun](https://github.com/kunkunsh/kunkun) (at
`vendors/space-lens`) and Xross (at `vendors/space-lens`) as exactly
pinned git submodules. Consumers pin a commit SHA, never a floating
branch.

### KFS consumer compatibility

| KFS revision | Schema | Kunkun pin | Xross pin |
|---|---|---|---|
| tag `kfs-v0.1.2` on this repository (supersedes `HuakunShen/file-search` `v0.1.1` = `0cfe7c27e25eea3b841f481bdf671575f35f5d0c`) | 1 | branch `file-search-submodule` | branch `file-search-vendor` |

`v0.1.0` (`e21a1de8`) is a historical snapshot superseded before any
consumer pinned it (NAPI generator alignment to `@napi-rs/cli` 3.6.2).
