...
standalone NPX/server mode
  ...
  exposes SpaceLensAPI-shaped endpoints over HTTP
  prefer Hono for the Node-compatible HTTP server
  browser connects with fetch-based HTTP client

Kunkun plugin mode
  deferred until after NPX mode works
  Kunkun custom view should eventually load the same SPA
  frontend will eventually call spawnBackend()
  backend will eventually expose SpaceLensAPI with exposeBackend()
  ...
  src/
    lib/
      api/
        ...
        http-client.ts        # fetch-based client for NPX/Hono standalone mode
        websocket-client.ts   # optional future kkrpc/ws client adapter
...
- The standalone host in `apps/tui` can serve the static build output and expose separate HTTP API endpoints.
...
- `apps/tui/package.json` owns the standalone NPX server dependencies, including `hono` and `@hono/node-server`.
...
Use `SpaceLensAPI` as the primary frontend abstraction. The first production transport should be HTTP for NPX standalone mode. KKRPC remains the preferred future transport for Kunkun/backend stdio or bidirectional streaming needs.

NPX HTTP server recommendation:
...
HTTP should preserve the exact `SpaceLensAPI` request/response DTOs so the UI does not care whether the implementation is demo, HTTP, or future KKRPC.

Future KKRPC import strategy:
...
- HTTP mode if URL has something like `?spaceLensApi=auto` or `?spaceLensApi=http://127.0.0.1:PORT/api`.
...
- Add demo client and HTTP client stub.
- Ensure UI only talks to the API abstraction.

### Milestone 3: NPX Standalone HTTP Backend
...
- Expose `SpaceLensAPI`-shaped JSON endpoints under `/api/*`.
- Add a fetch-based HTTP client adapter in the web app.
...
- SwiftUI native app.
- Kunkun plugin packaging before NPX HTTP mode works.
...
- The NPX/local HTTP server lives in `apps/tui`, not `apps/web`.
- The design can later run in Kunkun using `spawnBackend()` without rewriting the UI, but first implementation should prioritize NPX/Hono HTTP mode.
...
- The frontend must use a SpaceLensAPI/client abstraction so it can switch between demo, NPX HTTP/Hono, and future Kunkun spawnBackend transports.
...
- Do not put the HTTP server in apps/web; put NPX/Hono hosting code in apps/tui.
