...
- Transport must be abstracted. The first real integration target is NPX standalone mode over a local HTTP server. Kunkun backend/stdout-stdin RPC should be deferred, with interfaces kept ready for it.
- The UI should use shadcn-svelte and Tailwind as the main design language. DaisyDisk and CleanMyMac are references for product behavior and chart concepts, not skins to clone.
...
- `apps/tui/` should own the NPX/local HTTP server because it is already the published CLI package.
...
- `apps/web/references/cleanmymac-spacelens.png` - CleanMyMac Space Lens reference for a friendlier storage-map workflow and list/detail balance.
...
KKRPC/Hono WebSocket references outside this repo:

- `/Users/hk/Dev/kkrpc/packages/kkrpc/__tests__/hono-websocket.test.ts` - executable test showing Hono WebSocket upgrade, `createHonoWebSocketHandler`, `RPCChannel`, and `webSocketClientTransport`.
- `/Users/hk/Dev/kkrpc/docs/src/content/docs/examples/hono-websocket.md` - documentation example for exposing a KKRPC API over Hono WebSocket.
...
- Use shadcn-svelte components and Tailwind utilities for ordinary app structure: cards, buttons, inputs, badges, progress, sheets, scroll areas, separators.
- Keep custom CSS small and focused on global theme tokens and specialized chart/context-menu rendering.
- Use DaisyDisk and CleanMyMac as interaction/visual references, but do not produce a UI that looks like a direct copy of either product.
- Prefer a modern shadcn-style product surface over a macOS-window clone.
- Dense, practical information layout.
- Sunburst may use cyan/teal/green/amber/accent colors; it does not need to match DaisyDisk's green/yellow/orange palette.
...
apps/web
  ...
  no HTTP server implementation and no server-only SvelteKit routes
  ...
  src/
    lib/
      api/
        ...
        browser-mode.ts       # detect demo/kkrpc/kunkun mode
        rpc-client.ts         # kkrpc/ws client for NPX/Hono standalone mode
...
The current implementation keeps the canonical contract in `apps/tui/src/web-api.ts` and exports it as `@space-lens/cli/web-api`. `apps/web/src/lib/api/types.ts` should re-export those DTOs instead of duplicating them.
...
- `apps/web` must remain frontend-only. Do not put the HTTP server in `apps/web`.
- `apps/web` should only produce static assets that another host can serve.
...
- The standalone host in `apps/tui` can serve the static build output and expose `/rpc` over KKRPC WebSocket.
- Kunkun plugin mode should be able to load the same built SPA as a custom view.

Package ownership requirements:

- `apps/web/package.json` should only contain frontend/static-SPA dependencies such as SvelteKit, Tailwind, D3, and browser-only client helpers.
- Do not add Hono, Elysia, Node server libraries, scanner bindings, or filesystem/server dependencies to `apps/web`.
- `apps/tui/package.json` owns the standalone NPX server dependencies, including `hono`, `@hono/node-server`, `ws`, and `kkrpc`.
- If a shared API contract is needed later, move DTO-only TypeScript into a shared package; keep runtime server code out of `apps/web`.
...
Use `SpaceLensAPI` as the primary frontend abstraction. The production transport for NPX standalone mode is KKRPC over WebSocket, mounted on the Hono server at `/rpc`. Kunkun plugin mode should expose the same `SpaceLensAPI` through a different KKRPC transport, such as stdio or a Kunkun IPC adapter, without changing page/component code.

NPX server recommendation:

- Implement the server in `apps/tui`, not `apps/web`.
- Prefer Hono over Elysia for the first NPX implementation because Hono works cleanly in Node.js and other runtimes, while Elysia is more Bun-centric.
- Use `@hono/node-server` for the Node-compatible server.
...
- For WebSocket RPC, refer to `/Users/hk/Dev/kkrpc/examples/websocket-demo/src/hono.ts`, `/Users/hk/Dev/kkrpc/packages/kkrpc/__tests__/hono-websocket.test.ts`, and `/Users/hk/Dev/kkrpc/docs/src/content/docs/examples/hono-websocket.md`.
- The Hono WebSocket pattern is `createHonoWebSocketHandler({ expose })` on the server and `wrap<SpaceLensAPI>(webSocketClientTransport({ url }))` on the browser/client side.
- Use Kunkun `spawnBackend()` and `@kunkunsh/api/backend` `exposeBackend()` for plugin mode.

The frontend must call a stable `SpaceLensAPI` interface. The UI must not directly know whether it is using KKRPC WebSocket, Kunkun stdio/backend IPC, or demo data.
...
- Add demo client and KKRPC WebSocket client.
- Ensure UI only talks to the API abstraction.

### Milestone 3: NPX Standalone KKRPC Backend

- Add server/CLI code under `apps/tui`.
- Add a `spacelens web` or equivalent subcommand that serves `apps/web` static output.
- Keep `apps/web` free of server code and server dependencies.
...
### Milestone 4: Kunkun Plugin Mode (Deferred)
...
- SwiftUI native app.
- Kunkun plugin packaging before NPX KKRPC mode works.
- Placing the HTTP server in `apps/web`.
...
- The NPX/local Hono server lives in `apps/tui`, not `apps/web`.
- The frontend uses KKRPC WebSocket in NPX mode and can later run in Kunkun with a different KKRPC transport without rewriting the UI.
...
- The frontend must use a SpaceLensAPI/client abstraction so it can switch between demo, NPX KKRPC/Hono, and future Kunkun spawnBackend transports.
...
- Do not put the Hono/static/RPC server in apps/web; put NPX/Hono hosting code in apps/tui.
...
7. Add the kkrpc/ws client shape for the NPX/Hono server.
8. Add apps/tui Hono server skeleton with `/rpc` only after the frontend static UI is working.
